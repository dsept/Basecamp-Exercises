...
export default {
  ...
  setup() {
    ...
    const maxRevenue = computed(() => {
      return monthlyData.value.reduce((max, m) => {
        return m.revenue > max ? m.revenue : max
      }, 0)
    })
    ...
    const loadData = async () => {
      try {
        ...
        error.value = t('reports.errorLoad') + ': ' + err.message
      } finally {
        loading.value = false
      }
    }
    ...
    const formatMonth = (monthStr) => {
      // Convert YYYY-MM to localized format
    ...
    const getChangeValue = (current, previous) => {
      ...
      if (change > 0) {
        return '+' + currencySymbol.value + change.toLocaleString()
      } else if (change < 0) {
        return '-' + currencySymbol.value + Math.abs(change).toLocaleString()
      }
